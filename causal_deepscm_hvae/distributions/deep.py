...
class _DeepIndepNormal(DeepConditional):
    ...
    def predict(self, x) -> Independent:
        ...
        logvar = torch.nan_to_num(logvar, nan=_LOGVAR_MIN, posinf=_LOGVAR_MAX, neginf=_LOGVAR_MIN)
        logvar = logvar.clamp(min=_LOGVAR_MIN, max=_LOGVAR_MAX)
        std = torch.exp(0.5 * logvar).clamp_min(_EPS_SCALE)
